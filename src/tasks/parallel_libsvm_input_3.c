...
struct document_frequencies_hash *DF_prime = NULL;
int doc_max_count;
...
int paircmp (const void * p1, const void *p2) {
	if (((struct pair *)p1) -> v1 == ((struct pair *)p2) -> v1) return 0;
	else if (((struct pair *)p1) -> v1 < ((struct pair *)p2) -> v1) return 1;   //we aim for descending order
	else return -1;
}
...
void process_paragraph(char *words[], size_t number, xmlNodePtr node) {
	...
	double tfc_normalization_divisor = 1.0;

	struct pair pairs[1000];
	size_t pairs_index = 0;
	...
	for (i=0; i<number; i++) {
		...
		if (pairs_index < sizeof(pairs)/sizeof(struct pair)) {
			double tmp = word_entry->count;
			HASH_FIND_STR(DF_prime, words[i], word_entry);
			pairs[pairs_index].v1 = word_tf * word_idf;
			pairs[pairs_index].v2 = word_entry->count/tmp;
			pairs_index++;
		}
	}
	...
	//sort the pairs
	qsort(pairs, pairs_index, sizeof(struct pair), paircmp);

	if (recorded_bin || pairs_index) {// Some paras have no content words, skip.
		...
		//first: write down pairs
		for (i = 0; i < pairs_index; i++) {
			fprintf(result_file, " %ld:%f %ld:%.4f", 2*i, pairs[i].v1/tfc_normalization_divisor, 2*i+1, pairs[i].v2);
		}

		for (i=0; i<20000; i++) {
			if(bins[i] > 0.001) {
				// We also need to normalize on the basis of paragraph length
				fprintf(result_file, " %ld:%f",2*sizeof(pairs)/sizeof(struct pair)+i,bins[i]/tfc_normalization_divisor);
			}
		}
		fprintf(result_file,"\n");
	}
