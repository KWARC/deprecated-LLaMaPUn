...
static char * getAnnotationPtr(old_dnmPtr dnm, const char *annotation, int create);
...
static char ** getAnnotationList(old_dnmPtr dnm, char *string, size_t * final_length, size_t * final_number) {
...
static void copy_into_plaintext(const char *string, old_dnmPtr dnm, struct tmp_parsedata *dcs) {
...
static void parse_dom_into_dnm(xmlNode *n, old_dnmPtr dnm, struct tmp_parsedata *dcs, long parameters) {	
...
old_dnmPtr old_createDNM(xmlDocPtr doc, long parameters) {
	/* Creates a DNM and returns a pointer to it.
	   The memory has to be free'd later by calling old_freeDNM */
	...
	//dnm
	old_dnmPtr dnm = (old_dnmPtr)malloc(sizeof(struct old_dnm_struct));
	CHECK_ALLOC(dnm);
